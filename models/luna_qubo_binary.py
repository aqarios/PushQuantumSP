...
class QuboSPBinary:
    def __init__(self, gra, alpha=0.1, beta=0.1, gamma=1, P2=1, P3=1) -> None:
        self.gra = gra

        # Compute dynamic P1
        num_street_nodes = len([n for n in gra.G.nodes if len(n) == 3])
        num_lidar_nodes = len([n for n in gra.G.nodes if len(n) != 3])
        num_edges = gra.G.number_of_edges()

        P1 = (
            alpha * (num_lidar_nodes / num_street_nodes)
            + beta * np.log(num_edges + 1)
            + gamma
        )
    ...
    def invert_luna_solution(self, sample):
        converted_dict = {int(key[1:]): np.int8(value) for key, value in sample.items()}
        inverted = self.__inverter_matrix(converted_dict)
        return inverted
    
    def get_qubo_list(self):
